 ...
 */
export function socialAccountsCreateOAuthAuthorization(
  client: WoopSocialCore,
  request: models.CreateOAuthAuthorizationRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    models.CreateOAuthAuthorizationResponse,
    | WoopSocialError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(models.CreateOAuthAuthorizationRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/social-accounts/authorization-url")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.CreateOAuthAuthorizationResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
