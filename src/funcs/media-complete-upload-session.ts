...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.CompleteUploadSessionRequest$outboundSchema, value),
  ...
  const pathParams = {
    uploadSessionId: encodeSimple("uploadSessionId", payload.uploadSessionId, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/media/upload-sessions/{uploadSessionId}/complete")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.UploadSessionStatus$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
