...
export function socialAccountsGetSocialAccountPlatformInputs(
  ...
  Result<
    models.SocialAccountPlatformInputs,
    | WoopSocialError
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        operations.GetSocialAccountPlatformInputsRequest$outboundSchema,
        value,
      ),
  ...
  const pathParams = {
    socialAccountId: encodeSimple("socialAccountId", payload.socialAccountId, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/social-accounts/{socialAccountId}/platform-inputs")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.SocialAccountPlatformInputs$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
