...
import { WoopSocialDefaultError } from "../models/errors/woop-social-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | WoopSocialDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | WoopSocialDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new WoopSocialDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new WoopSocialDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
